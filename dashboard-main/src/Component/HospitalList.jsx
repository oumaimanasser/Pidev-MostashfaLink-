...
const getFullPageContent = () => {
  ...
  if (filteredHospitals.length > 0) {
    ...
    currentHospitals.forEach(hospital => {
      ...
      if (hospital.capacity === 0) content += "Actuellement complet. ";
      if (hospital.emergencyProtocol) content += "Protocole d'urgence activé. ";
    });
  } else {
    content += "Aucun hôpital trouvé. ";
  }
  
  // Ajouter le contenu des détails si un hôpital est sélectionné
  if (selectedHospital) {
    content += `Détails de l'hôpital ${selectedHospital.name}. `;
    content += `Adresse: ${selectedHospital.address}. `;
    content += `Téléphone: ${selectedHospital.phone}. `;
    content += `Capacité: ${selectedHospital.capacity} lits. `;
    if (selectedHospital.emergencyProtocol) {
      content += `Protocole urgence activé avec ${selectedHospital.emergencyCapacity} lits d'urgence. `;
    }
    
    if (hospitalInfos.length > 0) {
      content += "Informations supplémentaires: ";
      hospitalInfos.forEach(info => {
        content += `Services: ${info.services}. Départements: ${info.departments}. `;
      });
    }
  }
  ...
  return (
    <div className="hospital-container">
      <nav className="nav-bar">
        ...
        <Link to="/hospital-info" className="nav-link"> Informations détaillées</Link>
      </nav>
      <br/>
        ...
        {filteredHospitals.length > 0 ? (
          currentHospitals.map((h) => (
            ...
            >
              ...
              {h.emergencyProtocol && <p style={{ color: 'orange', fontWeight: 'bold' }}>Protocole urgence activé</p>}
