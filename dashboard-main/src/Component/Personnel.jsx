...
const Personnel = () => {
  ...
  onChange={(e) => {
    ...
    const updatedPersonnel = { ...newPersonnel, role };

    // 🧠 Suggestion automatique IA simple
    const now = new Date();
    if (role.toLowerCase() === 'médecin') {
      updatedPersonnel.shiftStart = new Date(now.setHours(8, 0, 0, 0));
      updatedPersonnel.shiftEnd = new Date(now.setHours(16, 0, 0, 0));
    } else if (role.toLowerCase() === 'infirmier') {
      updatedPersonnel.shiftStart = new Date(now.setHours(14, 0, 0, 0));
      updatedPersonnel.shiftEnd = new Date(now.setHours(22, 0, 0, 0));
    } else {
      updatedPersonnel.shiftStart = '';
      updatedPersonnel.shiftEnd = '';
    }

    setNewPersonnel(updatedPersonnel);
  }}
/>
